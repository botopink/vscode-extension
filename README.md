...
- Snippets for the most common declarations (`fn`, `val`, `type` with fields or variants, `behavior`, `case` with `Pattern { … }` arms, `loop` over a collection or a condition, `comptime`, `#[@iterator] fn`, `#[@result] fn … -> @Result<T, E>`, …) plus `test` blocks, `#[@External.Node(…), @External.Erlang(…)]` declares, and `import { … } from "std"`.
