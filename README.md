...
- Snippets for the most common declarations (`fn`, `val`, `type` with fields or variants, `behavior`, `case`, `loop` over a collection, `comptime`, `#[@iterator] fn`, `#[@result] fn … -> @Result<T, E>`, …) plus `test` blocks, `#[@External.Node(…), @External.Erlang(…)]` declares, and `import { … } from "std"`.
